...
diff_threshold_b1 = 250 #176.8118
...
def throttle_buy_volume(product, state, max_desired=10):
    current_pos = state.position.get(product, 0)
    limit = LIMIT[product]
    if current_pos >= limit: return 0  # Don’t buy if full
    return max(1, int(max_desired * (1 - (current_pos / limit))))
