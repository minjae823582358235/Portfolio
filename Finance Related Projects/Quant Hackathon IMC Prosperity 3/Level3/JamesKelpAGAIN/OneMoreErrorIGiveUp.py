...
class Strategy:
    ...
    def buy(self, price, quantity, state):
        for price, volume in get_best_asks_to_fill_WITH_LEVELS(self.product, state, IWantThisMuch): 
            self.orders.append(Order(self.product, int(price), volume))

    def sell(self, price, quantity, state):
        for price, volume in get_best_bids_to_fill_WITH_LEVELS(self.product, state, IWantThisMuch): 
            self.orders.append(Order(self.product, int(price), -volume))
...
class MarketMakingStrategy(Strategy):
    ...
    def act(self, state: TradingState) -> None:
        ...
        self.buy(price, quantity, state)
        self.sell(price, quantity, state)
...
################################################################################################################
###-----------     prosperity3bt "Level3/JamesKelpAGAIN/TralaleloTralala.py" 3 --no-out     -----------------###
################################################################################################################
