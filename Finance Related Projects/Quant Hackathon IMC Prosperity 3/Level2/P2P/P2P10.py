...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        def zscore(x: float, mean: float, stddev: float) -> float:
            if stddev == 0:
                return 0
            return (x - mean) / stddev

        def PriceAdjustment(product, mode=None):
            holdFactor = 12.3585  ## TODO OPTIMISE
            ...
            if product == "PICNIC_BASKET1":
                PB1_high = 37.2457 + holdPremium  # TODO OPTIMISE
                PB1_mid = 23.2929 + holdPremium  # TODO OPTIMISE
                PB1_low = 1.1353 + holdPremium  # TODO OPTIMISE
                PB1_neg = -25.7211 + holdPremium  # TODO OPTIMISE
                if mode == "buy":
                    factor = 1
                if mode == "sell":
                    factor = -1
            ...
            if product == "PICNIC_BASKET2":
                PB2_high = 31.0102 + holdPremium  # TODO OPTIMISE
                PB2_mid = 13.6235 + holdPremium  # TODO OPTIMISE
                PB2_low = -6.2591 + holdPremium  # TODO OPTIMISE
                PB2_neg = -18.7282 + holdPremium  # TODO OPTIMISE
                if mode == "buy":
                    factor = 1
                if mode == "sell":
                    factor = -1
            ...
            if product == "DJEMBES":
                DJ_high = 32.8658 + holdPremium  # TODO OPTIMISE
                DJ_mid = 3.4178 + holdPremium  # TODO OPTIMISE
                DJ_low = -4.8419 + holdPremium  # TODO OPTIMISE
                DJ_neg = -6.5678 + holdPremium  # TODO OPTIMISE
                if mode == "buy":
                    factor = 1
                if mode == "sell":
                    factor = -1
        ...
        zfactor1 = 1.5460  # TODO OPTIMISE!!!!!! 1 works pretty well
        s2offset = 105.417
        stdev2 = np.round(27.166 // np.sqrt(1000), decimals=5)
        zfactor2 = 0.7571  # TODO OPTIMISE!!!!!!
        s3offset = 26.51911  # PB1 value tracker
        stdev3 = np.round(27.0604 // np.sqrt(1000), decimals=5)
        pb1ZscoreThreshold = 1.2196
        pb2ZscoreThreshold = 0.5830
        # MAIN CHAIN OF LOGIC ##################################################
        ...
        for product in OrderbookDict:
            ...
            if product == "PICNIC_BASKET1":  # 6 croissants 3 jams 1 djembe
                ...
                normspread3 = spread3 - s3offset
                zscore3 = np.round(zscore(normspread3, 0, stdev3), decimals=5)
                ## represents the zscore fit of PB1
                zscore2 = np.round(zscore(normspread2, 0, stdev2), decimals=5)
                if (
                    normspread1
                    > stdev1
                    * zfactor1  ##Picnic Basket 1 is overvalued or PB2 OR Djembe is undervalued
                    ...
                    if normspread2 < stdev2 * zfactor2:
                        # assume this means PB2 is undervalued
                        if zscore2 < -pb2ZscoreThreshold:
                            # basic sanity check if overall market movement dominates
                            p2order.append(
                                Order(
                                    "PICNIC_BASKET2",
                ...
                if normspread1 < -stdev1 * zfactor1:
                    ##Picnic Basket 1 is undervalued or PB2 OR Djembe is overvalued
                    ## aggressive?
        ...
        logger.print("Spread 3 average: " + str(np.mean(self.spread3arr)))
        logger.print("Spread 3 stddev: " + str(np.sqrt(np.var(self.spread3arr))))
