    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        def PriceAdjustment(product, mode=None):
            ...
            if product == "PICNIC_BASKET1" or product == "PICNIC_BASKET2":
                highdump = 5 + holdPremium  # was 3
                middump = 2 + holdPremium  # was 0
                lowdump = -1 + holdPremium
                negativedump = -2 + holdPremium
                if mode == "buy":
                    factor = 1
                if mode == "sell":
                    factor = -1
                if VolumeFraction <= 0.1:
                    return int(factor * (highdump + 3))
            ...
            if product == "JAMS" or product == "CROISSANTS" or product == "DJEMBES":
                highdump = 2 + holdPremium  # was 2
                middump = 1 + holdPremium  # was 1
                lowdump = 0 + holdPremium  # was 0
                negativedump = 0 + holdPremium
                if mode == "buy":
                    factor = 1
                if mode == "sell":
                    factor = -1
        ...
        for product in OrderbookDict:
            ...
            if product == "PICNIC_BASKET1":  # 6 croissants 3 jams 1 djembe
                ...
                zfactor1 = 1  # TODO: check if this is correct raymond is going to optimise the z entry point. 1,0.5 previous
                ##############################
                spread1 = (
                    vwap("PICNIC_BASKET1")
                    - vwap("PICNIC_BASKET2")
