    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for product in OrderbookDict:
            if product == "PICNIC_BASKET1":  # 6 croissants 3 jams 1 djembe
                s1offset = -6610.328  # PB1 is usually cheaper than PB2
                stdev1 = 19.847 // np.sqrt(
                    1000
                )  # sigma np operation due to total std dev daily stddev
                zfactor1 = 0.5  # TODO: check if this is correct raymond is going to optimise the z entry point.
                ##############################
                spread1 = (
                    vwap("PICNIC_BASKET1")
                    - vwap("PICNIC_BASKET2")
