    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for product in OrderbookDict:
            if product == "PICNIC_BASKET1":  # 6 croissants 3 jams 1 djembe
                s1offset = 26.51911544355958  # theta
                stdev1 = 27.060413698355 // np.sqrt(
                    1000
                )  # sigma np operation due to total std dev daily stddev
                zfactor1 = 2  # TODO: check if this is correct raymond is going to optimise the z entry point.
                ##############################
                spread1 = (
                    vwap("PICNIC_BASKET1")
                    - 6 * vwap("CROISSANTS")
            ...
            if product == "PICNIC_BASKET2":  # 4 croissants 2 jams
                ...
                if (
                    ...
                    cheapestbid = BidPrice("PICNIC_BASKET2", mode="min")
                    cheapestVolume = BidVolume("PICNIC_BASKET2", mode="min")
                    p1order.append(
                        Order(
                            "PICNIC_BASKET2",
                            cheapestbid,
