# apply to all commodities and fine tune the params. excl kelp,resin,ink
import json
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        def PriceAdjustment(product, mode=None):
            ...
            if product == "PICNIC_BASKET1" or product == "PICNIC_BASKET2":
                highdump = 5 + holdPremium  # TODO OPTIMISE
                middump = 2 + holdPremium  # TODO OPTIMISE
                lowdump = -1 + holdPremium  # TODO OPTIMISE
                negativedump = -2 + holdPremium  # TODO OPTIMISE
                if mode == "buy":
                    factor = 1
                if mode == "sell":
                    factor = -1
                ...
                if VolumeFraction > 0.1 and VolumeFraction <= 0.2:
                    return int(factor * highdump)
                if VolumeFraction > 0.2 and VolumeFraction < 0.5:
                    return int(factor * middump)
                if VolumeFraction >= 0.5 and VolumeFraction < 1:
                    return int(factor * lowdump)
                if VolumeFraction >= 1:
                    return int(factor * negativedump)
            ###############
            if product == "JAMS" or product == "CROISSANTS" or product == "DJEMBES":
                highdump = 2 + holdPremium  # TODO OPTIMISE
                middump = 1 + holdPremium  # TODO OPTIMISE
                lowdump = 0 + holdPremium  # TODO OPTIMISE
                negativedump = 0 + holdPremium  # TODO OPTIMISE
                if mode == "buy":
                    factor = 1
                if mode == "sell":
                    factor = -1
                if VolumeFraction <= 0.1:
                    return factor * highdump
                if VolumeFraction > 0.1 and VolumeFraction < 0.5:
                    return factor * middump
                if VolumeFraction >= 0.5 and VolumeFraction < 1:
                    return factor * lowdump
                if VolumeFraction >= 1:
                    return factor * negativedump
        ...
        for product in OrderbookDict:
            ...
            if product == "PICNIC_BASKET1":  # 6 croissants 3 jams 1 djembe
                s1offset = -78.898151  # PB1 is usually cheaper than PB2
                stdev1 = 20.92556 // np.sqrt(
                    1000
                )  # sigma np operation due to total std dev daily stddev
                zfactor1 = 1  # TODO OPTIMISE!!!!!!
                ##############################
                spread1 = (
                ...
                if (
                    ...
                    cheapestbid = BidPrice("PICNIC_BASKET1", mode="min")
                    cheapestVolume = BidVolume("PICNIC_BASKET1", mode="min")

                    p1order.append(
                        Order(
                            "PICNIC_BASKET1",
                            cheapestbid
                            + PriceAdjustment("PICNIC_BASKET1", mode="sell"),
                            -cheapestVolume,
                        )
                    )
                    p2order.append(
                        Order(
                            "PICNIC_BASKET2",
                            AskPrice("PICNIC_BASKET2", mode="max")
                            + PriceAdjustment("PICNIC_BASKET2", mode="buy"),
                            AskVolume("PICNIC_BASKET2", mode="max"),
                        )
                    )
                    croissantorder.append(
                        Order(
                            "CROISSANTS",
                            AskPrice("CROISSANTS", mode="max")
                            + PriceAdjustment("CROISSANTS", mode="buy"),
                            AskVolume("CROISSANTS", mode="max"),
                        )
                    )
                    jamorder.append(
                        Order(
                            "JAMS",
                            AskPrice("JAMS", mode="max")
                            + PriceAdjustment("JAMS", mode="buy"),
                            AskVolume("JAMS", mode="max"),
                        )
                    )
                    djembeorder.append(
                        Order(
                            "DJEMBES",
                            AskPrice("DJEMBES", mode="max")
                            + PriceAdjustment("DJEMBES", mode="buy"),
                            AskVolume("DJEMBES", mode="max"),
                        )
                    )
                if (
                    normspread1 < -stdev1 * zfactor1
                ):  ## buy at the worst ask(most expensive) ##TODO maybe be more aggressive?
                    expensiveask = AskPrice("PICNIC_BASKET1", mode="max")
                    expensiveVolume = AskVolume("PICNIC_BASKET1", mode="max")
                    p1order.append(
                        Order(
                            "PICNIC_BASKET1",
                            expensiveask
                            + PriceAdjustment("PICNIC_BASKET1", mode="buy"),
                            expensiveVolume,
                        )
                    )
                    p2order.append(
                        Order(
                            "PICNIC_BASKET2",
                            BidPrice("PICNIC_BASKET2", mode="min")
                            + PriceAdjustment("PICNIC_BASKET2", mode="sell"),
                            -BidVolume("PICNIC_BASKET2", mode="min"),
                        )
                    )
                    croissantorder.append(
                        Order(
                            ...
                            + PriceAdjustment("CROISSANTS", mode="sell"),
                    ...
                    jamorder.append(
                        Order(
                            ...
                            + PriceAdjustment("JAMS", mode="sell"),
