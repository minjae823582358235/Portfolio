...
def project_next(smoothed, window=10, correlation_threshold=0.8):
    ...
    try:
        ...
        if correlation >= correlation_threshold:
            ...
            logger.print(f"Linear fit used with correlation: {correlation:.2f}")
        else:
            ...
            logger.print(f"Quadratic fit used as linear correlation was {correlation:.2f} (below threshold).")
...
class Trader: ##from here
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        if state.timestamp >= 100*50:
            ...
            BasePB1Future=project_next(pb1Smoothed,window=int(PBWindow),correlation_threshold=0.7)
            pb2Smoothed=kalman_filter_1d(self.pb2_mid_prices)
            SynthPB2Future=4*futurePriceDict[CROISSANTS]+2*futurePriceDict[JAMS]
            BasePB2Future=project_next(pb2Smoothed,window=int(PBWindow),correlation_threshold=0.7)    
            PotentialReturns1=self.calculate_realized_returns(PICNIC_BASKET1,state=state)
            PotentialReturns2=self.calculate_realized_returns(PICNIC_BASKET2,state=state)
            futurePriceDict[PICNIC_BASKET1]=BasePB1Future
            futurePriceDict[PICNIC_BASKET2]=BasePB2Future
            pb1Bool=PotentialReturns1>1000 or PotentialReturns1 < -500 
            pb2Bool=PotentialReturns2>1000 or PotentialReturns2<-500
            TruthDict={PICNIC_BASKET1:pb1Bool,PICNIC_BASKET2:pb2Bool}
            LHS=np.round(futurePriceDict[PICNIC_BASKET2]+2*futurePriceDict[CROISSANTS]+futurePriceDict[DJEMBES]+futurePriceDict[JAMS],decimals=4)
            if pb1Bool or pb2Bool:
                for pb in [PICNIC_BASKET1,PICNIC_BASKET2]:
                    if TruthDict[pb]:
                        if self.position[pb]>0:
                            result[pb]=self.martingaleOrders(pb,futurePriceDict[pb],state.order_depths[pb],mode='sell')
                            logger.print('selloff triggered')
                        elif self.position[pb]<0:
                            result[pb]=self.martingaleOrders(pb,futurePriceDict[pb],state.order_depths[pb],mode='buy')
                            logger.print('buyoff triggered')
            else:
                threshold=np.round(0.882368,decimals=3)*2
                if LHS>futurePriceDict[PICNIC_BASKET1]+threshold:
                    result[PICNIC_BASKET1]=self.martingaleOrders(PICNIC_BASKET1,futurePriceDict[PICNIC_BASKET1],state.order_depths[PICNIC_BASKET1],mode='buy')
                    result[PICNIC_BASKET2]=self.martingaleOrders(PICNIC_BASKET2,futurePriceDict[PICNIC_BASKET2],state.order_depths[PICNIC_BASKET2],mode='sell')
                    logger.print('lhs overvalued triggered')
                elif LHS<futurePriceDict[PICNIC_BASKET1]-threshold:
                    result[PICNIC_BASKET1]=self.martingaleOrders(PICNIC_BASKET1,futurePriceDict[PICNIC_BASKET1],state.order_depths[PICNIC_BASKET1],mode='sell')
                    result[PICNIC_BASKET2]=self.martingaleOrders(PICNIC_BASKET2,futurePriceDict[PICNIC_BASKET2],state.order_depths[PICNIC_BASKET2],mode='buy')
                    logger.print('pb1 overvalued triggered')
