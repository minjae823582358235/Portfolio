...
class Trader: ##from here
    ...
    def __init__(self) -> None:
        ...
        self.sunDeltatolerance=0.011 #0.01
        self.sunMode=None
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for product in [KELP,SQUID_INK,JAMS,DJEMBES]:
            if state.order_depths[product].buy_orders or state.order_depths[product].sell_orders:
                ...
                if product==KELP:
                    self.kelp_mid_prices.append(midP)
                    ...
                    self.ink_mid_prices.append(midP)
                    ...
                    self.jam_mid_prices.append(midP)
                    ...
                    self.djembe_mid_prices.append(midP)
                    midParr=self.jam_mid_prices
                    DThresh=1 # was 2
                    prodThresh=DThresh
                    Dorder=2
                ...
                if state.timestamp>=100*50:
                    ...
                    Kmode=None
                    if nextPrice>midP+prodThresh:
                        Kmode='sell'
                        result[product]+=self.martingaleOrders(product,nextPrice,ProdOD,Kmode)
                        logger.print(f'sell signal triggered')
                    elif nextPrice<midP-prodThresh:
                        Kmode='buy'
                        logger.print(f'buy signal triggered')
                        result[product]+=self.martingaleOrders(product,nextPrice,ProdOD,Kmode)
                    else:
