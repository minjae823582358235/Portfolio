                ...
                if state.timestamp>=100*50:
                    ...
                    if abs(signal)>self.signalTrigger[product]:
                        if signal>0:
                            ...
                            result[product]+=self.martingaleOrders(product,nextPrice,ProdOD,Kmode)
        for product in self.triggerlist:
            if product==DJEMBES or product == CROISSANTS:
                logger.print(product+'mean signal: '+ str(np.mean(self.triggerlist[product])))
                logger.print(product+'stdev signal: '+ str(np.sqrt(np.var(self.triggerlist[product]))))
