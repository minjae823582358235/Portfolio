...
class Trader: ##from here
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for strategy_key, strategy in self.strategies.items():
            ...
            if isinstance(strategy_state, dict):
                ...
                # except Exception as e: logger.print(f"Error loading state for {strategy_key}: {e}")
                    ...
                    # logger.print(f"Error loading state for {strategy_key}: {e}")
                    continue
        ...
        # CHECK IF WE WANT TO DELTA HEDGE, WE GET 50K ON MEAN REVERSION - Delta hedge with ±hedge_band
        # net_delta = cur_port_delta + total_delta_imp
        # band = data['params']['hedge_band']
        # if abs(net_delta) > band:
        #     raw = -net_delta
        #     hedge_qty = int(math.copysign(abs(raw)-band, raw))
        #     max_buy  = POSITION_LIMITS[VOLCANIC_ROCK] - cur_under
        #     max_sell = -POSITION_LIMITS[VOLCANIC_ROCK] - cur_under
        #     hedge_qty = max(min(hedge_qty, max_buy), max_sell)
        #     if hedge_qty:
        #         price = best_ask if hedge_qty>0 else best_bid
        #         result[VOLCANIC_ROCK].append(
        #             Order(VOLCANIC_ROCK, int(price), hedge_qty)
        #         )
