        ...
        for product in [KELP,SQUID_INK,JAMS,DJEMBES,CROISSANTS]:
            ...
            if state.timestamp >= 200:
                if midParr[-1]>midParr[-2]:
                    self.signalDict[product]+=midParr[-1]-midParr[-2]
                ...
                if state.timestamp>=100*50:
                    if self.positionCounter[product]>5 and self.calculate_realized_returns(product,state)>0:
                        if self.position[product]>0:
                            result[product]=self.martingaleOrders(product,midParr[-1]-2,state.order_depths[product],mode='sell',volume=abs(self.position[product]))
                        elif self.position[product]<0:
                            result[product]=self.martingaleOrders(product,midParr[-1]+2,state.order_depths[product],mode='buy',volume=abs(self.position[product]))
                    else:
                        smoothedmid=kalman_filter_1d(midParr[-100:])
                        nextPrice=int(project_next(smoothedmid,window=10))
                        Kmode=None
