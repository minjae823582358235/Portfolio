            ...
            for pb in [PICNIC_BASKET1,PICNIC_BASKET2]:
                ...
                if self.positionCounter[pb]>10 and self.calculate_realized_returns(pb,state)>0:
                    if self.position[pb]>0:
                        result[pb]=self.martingaleOrders(pb,pbVal,state.order_depths[pb]-2,mode='sell',volume=self.position[pb])
                    elif self.position[pb]<0:
                        result[pb]=self.martingaleOrders(pb,pbVal,state.order_depths[pb]+2,mode='buy',volume=self.position[pb])
                else:
