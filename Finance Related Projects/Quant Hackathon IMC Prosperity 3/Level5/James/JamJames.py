...
################################################################################################################
    ...
    def update_market_data(self, product, state):
        order_depth = state.order_depths[product]
        obvs = state.observations.conversionObservations[JAMS]
...
################################################################################################################
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        can_convert = min(abs(self.position[JAMS]), ASPARAMS[JAMS]["max_order_size"])
        conversions = 0
        trader_data = ""
        ...
        local_best_bid = max(state.order_depths[JAMS].buy_orders.keys())
        local_best_ask = min(state.order_depths[JAMS].sell_orders.keys())
        ...
        delta = avellaneda_stoikov_delta(JAMS, fv, self.position[JAMS])


        correlation = self.compute_pearson_correlation()
        
        if abs(correlation) < PARAMS[JAMS]["min_corr_threshold"]:
...
###############################################################################################################
        ...
        action, current_residual, z = self.calculate_zscore_action(
            ...
            z_threshold=PARAMS[JAMS]["z_threshold"],
        )


        if action == "buy" and island2_ask < fv:
            result[JAMS].append(Order(JAMS, fv, +self.lot_size))
            if self.position[JAMS] > 0:
...
################################################################################################################
        ...
        logger.print(f"[DEBUG] z={z:.2f}, act={action}, fv={fv:.2f}, i2ask={island2_ask:.2f}, i2bid={island2_bid:.2f}, pos={self.position[JAMS]}")
