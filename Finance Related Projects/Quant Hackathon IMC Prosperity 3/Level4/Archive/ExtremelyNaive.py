...
class Trader: ##from here
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        if MAGNIFICENT_MACARONS in state.order_depths:
            ...
            # Detect and store regime shifts
            self.factor_history["sugarfactor"].append(self.sugarfactor)
            self.factor_history["exportfactor"].append(self.exportfactor)
            self.factor_history["importfactor"].append(self.importfactor)
            self.factor_history["transportfactor"].append(self.transportfactor)
            self.factor_history["sunfactor"].append(self.sunfactor)

            regime_alerts = self.detect_regime_shift()
            for label, value, z in regime_alerts:
                logger.print(f"🚨 Regime Shift Detected for {label}: Coef={value:.4f}, Z={z:.2f}")

            # Compute adaptive fair value
            outsideMidPrice = (outsideMacaronDict[BID] + outsideMacaronDict[ASK]) / 2
            factor_map = {
                "sugarfactor": (feeDict[SUGAR], self.sugarfactor),
                "exportfactor": (feeDict[ETARIFF], self.exportfactor),
                "importfactor": (feeDict[ITARIFF], self.importfactor),
                "transportfactor": (feeDict[TRANSPORT], self.transportfactor),
                "sunfactor": (feeDict[SUNLIGHT], self.sunfactor),
            }

            adjusted_fair_value = outsideMidPrice
            for factor, (input_val, weight) in factor_map.items():
                decay = 0.2 if self.cooldown_factors[factor] > 0 else 1.0
                adjusted_fair_value += decay * input_val * weight
            fairValue = np.round(adjusted_fair_value, decimals=4)

            # Decide if trade is viable
            price_diff = macMid - fairValue
            mac_spread_threshold = 2.0
            if abs(price_diff) < mac_spread_threshold:
                logger.print(f"⚠️ Skipping MACARONS: FairValue={fairValue}, macMid={macMid}, diff={price_diff:.2f}")
            else:
                logger.print(f"✅ MACARONS Trade Signal: FV={fairValue}, MP={macMid}, Δ={price_diff:.2f}")
                if price_diff > 0:
                    available = min(
                        BidVolume(MAGNIFICENT_MACARONS, mode='max', state=state),
                        VolumeCapability(MAGNIFICENT_MACARONS, mode='sell', state=state)
                    )
                    size = max(min(int(available * 0.5), LIMIT[MAGNIFICENT_MACARONS]["limit"]), 1)
                    result[MAGNIFICENT_MACARONS] += self.OrderOptimised(MAGNIFICENT_MACARONS, size, mode='sell', state=state)
                else:
                    available = min(
                        AskVolume(MAGNIFICENT_MACARONS, mode='min', state=state),
                        VolumeCapability(MAGNIFICENT_MACARONS, mode='buy', state=state)
                    )
                    size = max(min(int(available * 0.5), LIMIT[MAGNIFICENT_MACARONS]["limit"]), 1)
                    result[MAGNIFICENT_MACARONS] += self.OrderOptimised(MAGNIFICENT_MACARONS, size, mode='buy', state=state)

            # Emergency rebalance (respects limit)
            buy_cap = VolumeCapability(MAGNIFICENT_MACARONS, mode='buy', state=state)
            sell_cap = VolumeCapability(MAGNIFICENT_MACARONS, mode='sell', state=state)

            if buy_cap < 26:
                rebalance_size = min(40, sell_cap)
                if rebalance_size > 0:
                    logger.print("🧯 Rebalancing: forced sell")
                    result[MAGNIFICENT_MACARONS] += self.OrderOptimised(MAGNIFICENT_MACARONS, rebalance_size, mode='sell', state=state)
            elif sell_cap > 26:
                rebalance_size = min(40, buy_cap)
                if rebalance_size > 0:
                    logger.print("🧯 Rebalancing: forced buy")
                    result[MAGNIFICENT_MACARONS] += self.OrderOptimised(MAGNIFICENT_MACARONS, rebalance_size, mode='buy', state=state)

        # Decrement cooldowns
        for factor in self.cooldown_factors:
            if self.cooldown_factors[factor] > 0:
                self.cooldown_factors[factor] -= 1
        if VolumeCapability(MAGNIFICENT_MACARONS,mode='buy',state=state) <65:
            conversions=-10
        if VolumeCapability(MAGNIFICENT_MACARONS,mode='sell',state=state) <65:
            conversions=10
