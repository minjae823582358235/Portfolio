...
def project_next_linear(smoothed, window=10):
    """
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        if MAGNIFICENT_MACARONS in state.order_depths:
            ...
            if len(self.macaron_mid_prices)>=20:
                ...
                smoothed=kalman_filter_1d(self.macaron_fair_prices)
                nextmidprice=project_next_linear(smoothed=smoothed,window=10)  
                # smoothed=kalman_filter_1d(self.macaron_mid_prices)
                # nextmidprice=project_next_linear(smoothed=smoothed,window=20)
                ...
                if nextmidprice > macMid: #sell orders
                    result[MAGNIFICENT_MACARONS]+=[
                        Order(MAGNIFICENT_MACARONS,int(nextmidprice-abs(gap)),-abs(macOD.buy_orders[int(macMid-abs(gap))])) for gap in bidGaps
                    ]
                elif nextmidprice < macMid: # buy orders:
                    result[MAGNIFICENT_MACARONS]+=[
                        Order(MAGNIFICENT_MACARONS,int(nextmidprice+abs(gap)),abs(macOD.sell_orders[int(abs(gap)+macMid)])) for gap in askGaps
                    ]  
                else: #conversion if worth it to avoid the storage fees
                    pass
            
            
            # if VolumeCapability(MAGNIFICENT_MACARONS,mode='buy',state=state) <10:
            #     conversions=-10
            # if VolumeCapability(MAGNIFICENT_MACARONS,mode='sell',state=state) <-10:
            #     conversions=10
