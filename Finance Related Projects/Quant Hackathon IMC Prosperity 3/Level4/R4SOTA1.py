...
class Trader: ##from here
    ...
    # TIDY THIS AFTER OPTIONS STRAT FINALISED
    def _initialize_state(self) -> Dict:
        return {
            'params': {
                'base_iv_window': 50,
                'epsilon_scale': 50,
                'delta_limit': POSITION_LIMITS[VOLCANIC_ROCK],
                'hedge_band': 500,
            },
            'smile_params':    {'a':0,'b':0,'c':0},
            'base_iv_history': [],
            'last_smile_v':    {},
            'deltas':          {}
        }

    def _update_time_to_expiry(self, timestamp: int) -> float:
        day = timestamp // 1_000_000
        base = 8 - day
        it = (timestamp % 1_000_000)//100
        adj = it/10000
        return max(0.0001, (base-adj)/365)

    def _mid_price(self, od: OrderDepth) -> float:
        if not od.buy_orders or not od.sell_orders:
            return None
        return (max(od.buy_orders.keys()) + min(od.sell_orders.keys()))/2

    def _bs_call_price(self, S: float, K: float, T: float, vol: float) -> float:
        if vol<=0 or T<=0:
            return max(S-K,0)
    ...
    def _bs_call_delta(self, S: float, K: float, T: float, vol: float) -> float:
        if vol<=0 or T<=0:
            return 1.0 if S>K else 0.0
        d1 = (math.log(S/K)+0.5*vol*vol*T)/(vol*math.sqrt(T))
        return 0.5*(1+math.erf(d1/math.sqrt(2)))

    def _implied_vol(self, S: float, K: float, T: float, mp: float) -> float:
        low,high=1e-6,5.0
        for _ in range(50):
            mid=(low+high)/2
            p=self._bs_call_price(S,K,T,mid)
            if p>mp: high=mid
            else: low=mid
        return (low+high)/2

    def _fit_parabola(self, m: List[float], v: List[float]) -> tuple:
        a,b,c = np.polyfit(m,v,2)
        return float(a),float(b),float(c)

    def _gen_option_orders(self,
                           product: str,
                           eps: float,
                           mid: float,
                           cur: int,
                           ...
                           delta_pc: float,
                           cur_del: float,
                           delta_lim: float) -> (List[Order], float):
        side = -1 if eps<0 else 1
        raw = abs(eps)*scale
        qty = max(1, math.ceil(raw)) if raw>0 else 0
        maxp = lim - cur*side
        tgt = delta_lim*(1 if eps>=0 else -1)
        cap = (tgt-cur_del)/(delta_pc or 1e-6)
        qty = min(qty,int(cap),maxp)
        if qty>0:
            return [Order(product, mid, side*qty)], side*qty*delta_pc
        return [],0.0

    def _gen_base_iv_straddle(self,
                               S: float,
                               mid: float,
                               cur: int,
                               lim: int,
                               data: Dict) -> List[Order]:
        hist=data['base_iv_history']
        if len(hist)<data['params']['base_iv_window'] or mid is None:
            return []
        avg=float(np.mean(hist))
        c = data['smile_params']['c']
        diff=c-avg
        th=data['params'].get('base_straddle_thresh',0.02)
        if abs(diff)<th: return []
        side=1 if diff<0 else -1
        raw=int(abs(diff)*data['params']['epsilon_scale'])
        qty=min(raw,lim-cur*side)
        if qty>0:
            return [Order(VOLCANIC_ROCK_VOUCHER_10000, mid, side*qty)]
        return []
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        # -------- VOLCANIC ROCK STRATEGY: OLDER VERSION --------
        ...
        # -------- Volcanic Rock Strategy - UPDATED BUT CHECK FOR ROUND 4 --------
        data = jsonpickle.decode(state.traderData) if state.traderData else self._initialize_state()
        result = {p: [] for p in state.order_depths}

        # Underlying mid price
        od_u = state.order_depths.get(VOLCANIC_ROCK)
        if not od_u or not od_u.buy_orders or not od_u.sell_orders:
            return result, 0, jsonpickle.encode(data)
        best_bid = max(od_u.buy_orders.keys())
        best_ask = min(od_u.sell_orders.keys())
        rock_mid = (best_bid + best_ask) / 2
        S_t = rock_mid

        # Time to expiry
        TTE = self._update_time_to_expiry(state.timestamp)

        # Gather smile points & deltas
        m_list, v_list, deltas = [], [], {}
        mid_prices = {}
        for p, K in STRIKES.items():
            od = state.order_depths.get(p)
            if not od: continue
            mid = self._mid_price(od)
            if mid is None: continue
            m = math.log(K/S_t)/math.sqrt(TTE)
            try:
                v = self._implied_vol(S_t, K, TTE, mid)
            except:
                continue
            data['last_smile_v'][p] = v
            m_list.append(m)
            v_list.append(v)
            deltas[p] = self._bs_call_delta(S_t, K, TTE, v)
            mid_prices[p] = mid
        data['deltas'] = deltas

        # Fit parabola
        if len(m_list) >= 3:
            a,b,c = self._fit_parabola(m_list, v_list)
            data['smile_params'] = {'a':a,'b':b,'c':c}
        else:
            a = data['smile_params']['a']
            b = data['smile_params']['b']
            c = data['smile_params']['c']

        # Base IV history
        data['base_iv_history'].append(c)
        if len(data['base_iv_history']) > data['params']['base_iv_window']:
            data['base_iv_history'].pop(0)

        # Portfolio delta
        cur_opt_delta = sum(state.position.get(p,0)*deltas.get(p,0) for p in STRIKES)
        cur_under = state.position.get(VOLCANIC_ROCK,0)
        cur_port_delta = cur_opt_delta + cur_under

        # Cross-sectional trades (excluding 10500)
        total_delta_imp = 0.0
        for p, K in STRIKES.items():
            mid = mid_prices.get(p)
            if mid is None: continue
            m = math.log(K/S_t)/math.sqrt(TTE)
            v = data['last_smile_v'][p]
            model_v = a*m*m + b*m + c
            eps = v - model_v
            orders, d_imp = self._gen_option_orders(
                p, eps, mid,
                state.position.get(p,0),
                POSITION_LIMITS[p],
                data['params']['epsilon_scale'],
                deltas[p],
                cur_port_delta + total_delta_imp,
                data['params']['delta_limit']
            )
            for o in orders:
                o.price = int(round(o.price))
            result[p].extend(orders)
            total_delta_imp += d_imp

        # ATM straddle on 10000 only
        mid10000 = mid_prices.get(VOLCANIC_ROCK_VOUCHER_10000)
        if mid10000 is not None:
            atm_orders = self._gen_base_iv_straddle(
                S_t, mid10000,
                state.position.get(VOLCANIC_ROCK_VOUCHER_10000,0),
                POSITION_LIMITS[VOLCANIC_ROCK_VOUCHER_10000],
                data
            )
            for o in atm_orders:
                o.price = int(round(o.price))
            result[VOLCANIC_ROCK_VOUCHER_10000].extend(atm_orders)
            for o in atm_orders:
                total_delta_imp += o.quantity * self._bs_call_delta(
                    S_t, STRIKES[VOLCANIC_ROCK_VOUCHER_10000], TTE,
                    data['last_smile_v'][VOLCANIC_ROCK_VOUCHER_10000]
                )
            ...
            for product, (mid_prices, o_depth, pos, pos_limit) in rock_price_dict.items():
                if product == VOLCANIC_ROCK:
                    ...
                    vrock_position = state.position.get(product, 0)
                    vrock_order_depth = state.order_depths[product]
                    vrock_fair_value = int(mid_prices[-1])

                    buy_volume = pos_limit - pos
                    sell_volume = -pos_limit - pos
                    
                    # When price below 3 buy, if greater sell
                    if vrock_fair_value <= 3:
                        result[product].append(Order(product, 3, buy_volume))
                    elif vrock_fair_value > 3:
                        result[product].append(Order(product, 4, sell_volume))
        # --------------------------------------------------------------------------
        ...
        if PICNIC_BASKET1 in state.order_depths:
            if (diff_comp1 < self.diff_threshold_b1): 
                ...
                for price, volume in get_best_asks_to_fill_WITH_LEVELS(PICNIC_BASKET1, state, desired_volume(PICNIC_BASKET1, state=state)):
                    result[PICNIC_BASKET1].append(Order(PICNIC_BASKET1, price, -volume))  # CHECK IF NEGATIVE 
            ...
            if (
                normspread2 < synth2Sigma * s2Zscore
                    ...
                    for price, volume in get_best_asks_to_fill_WITH_LEVELS(PICNIC_BASKET2, state, IWantThisMuchFor2):
                        result[PICNIC_BASKET2].append(Order(PICNIC_BASKET2, price, -volume))  # CHECK IF NEGATIVE
            elif (
                normspread2 > synth2Sigma * s2Zscore
        ...
        if MAGNIFICENT_MACARONS in state.order_depths:
            ...
            macpriceSmoothed=kalman_filter_1d(self.macaron_mid_prices[-500:])
            if len(macpriceSmoothed) >= 5:
                nextmacprice = project_next(macpriceSmoothed, window=5, order=1)
            else:
                nextmacprice = macMid  # Fallback to current mid-price
            if state.timestamp !=0:
                sunDelta=feeDict[SUNLIGHT]-self.previousSunPrice
                deltaSharpness=sunDelta-self.previousSunDelta
                macOD=state.order_depths[MAGNIFICENT_MACARONS]
                askGaps=[askPrice-macMid for askPrice in macOD.sell_orders]
                bidGaps=[macMid-bidPrice for bidPrice in macOD.buy_orders]
                bid = max(macOD.buy_orders)
                ask = min(macOD.sell_orders)
                patience=int(macpatience)
                if self.positionCounter[MAGNIFICENT_MACARONS]>patience:
                    if state.position[MAGNIFICENT_MACARONS]>0:
                        conversions=max(-10,-state.position[MAGNIFICENT_MACARONS])
                    elif state.position[MAGNIFICENT_MACARONS]<0:
                        conversions=min(10,-state.position[MAGNIFICENT_MACARONS])
                else: 
                    if abs((macMid-np.mean(macpriceSmoothed[:-1]))/np.sqrt(np.var(macpriceSmoothed[:-1])))>2:
                        window = list(self.macaron_mid_prices)[-self.mean_window:]
                        mu = float(np.mean(window))
                        sigma = float(np.std(window))
                        upper = mu + self.zscore * sigma
                        lower = mu - self.zscore * sigma
                        pos = state.position.get(MAGNIFICENT_MACARONS, 0)
                        size = self.trade_size
                        if macMid > upper:
                            result[MAGNIFICENT_MACARONS].append(Order(MAGNIFICENT_MACARONS, bid, -size))
                        elif macMid < lower:
                            result[MAGNIFICENT_MACARONS].append(Order(MAGNIFICENT_MACARONS, ask, size))
                        elif lower <= macMid <= upper and pos != 0:
                            if pos > 0:
                                result[MAGNIFICENT_MACARONS].append(Order(MAGNIFICENT_MACARONS, bid, -pos))
                            else:
                                result[MAGNIFICENT_MACARONS].append(Order(MAGNIFICENT_MACARONS, ask, pos))
                            self.sunMode = 'waiting'
                    else:
                        dumpGreed=macdumpGreed
                        standardGreed=macstandardGreed
                        if abs(deltaSharpness)<=self.sunDeltatolerance:
                            if self.Profitable(MAGNIFICENT_MACARONS,state=state,greed=dumpGreed,mode='buy'):
                                conversions=min(10,-state.position[MAGNIFICENT_MACARONS])
                            if self.Profitable(MAGNIFICENT_MACARONS,state=state,greed=dumpGreed,mode='sell'):
                                conversions=max(-10,-state.position[MAGNIFICENT_MACARONS])
                            if self.sunMode=='SELL':
                                if self.Profitable(MAGNIFICENT_MACARONS,state=state,greed=standardGreed,mode='buy'):
                                    result[MAGNIFICENT_MACARONS]+=[
                                    Order(MAGNIFICENT_MACARONS,int(nextmacprice+abs(gap)),abs(macOD.sell_orders[price])) for gap,price in zip(askGaps,macOD.sell_orders)
                                    ]     
                                result[MAGNIFICENT_MACARONS]+=[
                                Order(MAGNIFICENT_MACARONS,int(nextmacprice-abs(gap)),-abs(macOD.buy_orders[price])) for gap,price in zip(bidGaps,macOD.buy_orders)
                                ]
                            if self.sunMode=='BUY':
                                if self.Profitable(MAGNIFICENT_MACARONS,state=state,greed=standardGreed,mode='sell'):
                                    result[MAGNIFICENT_MACARONS]+=[
                                    Order(MAGNIFICENT_MACARONS,int(nextmacprice-abs(gap)),-abs(macOD.buy_orders[price])) for gap,price in zip(bidGaps,macOD.buy_orders)
                                    ]
                                result[MAGNIFICENT_MACARONS]+=[
                            ...
                            if deltaSharpness<0: # sudden increase in sunlight rate -> decrease in macaron price imminent
                                result[MAGNIFICENT_MACARONS]+=[
                                    Order(MAGNIFICENT_MACARONS,int(nextmacprice-abs(gap)),-abs(macOD.buy_orders[price])) for gap,price in zip(bidGaps,macOD.buy_orders)
                                ]
                                self.sunMode='BUY'
                            else: #sudden decrease in sunlight rate -> increase in macaron price imminent
                                result[MAGNIFICENT_MACARONS]+=[
                                Order(MAGNIFICENT_MACARONS,int(nextmacprice+abs(gap)),abs(macOD.sell_orders[price])) for gap,price in zip(askGaps,macOD.sell_orders)
                                ]  
                                self.sunMode='SELL'
                self.previousSunDelta=sunDelta
            self.previousSunPrice=feeDict[SUNLIGHT]
