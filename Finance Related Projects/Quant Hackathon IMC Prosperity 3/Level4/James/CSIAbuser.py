...
PARAMS = {
    ...
    MAGNIFICENT_MACARONS: {
        ...
        "CRITICAL_SUNLIGHT" : 20, #either 58.25 or 45   # threshold where macarons surge
        "SUN_BUFFER" : 0    # hysteresis so we don’t flip‑flop
...
################################################################################################################
    def run(self, state: TradingState) -> Tuple[Dict[Symbol, List[Order]], int, str]:
        ...
        pos_limit = LIMIT[MAGNIFICENT_MACARONS]

        CSI       = PARAMS[MAGNIFICENT_MACARONS]["CRITICAL_SUNLIGHT"]
        edge      = max(0, CSI - sunlight)             # how far below CSI
        # lot_ex    = min(max(10, int(edge * 5)), 50)     # 10‑50 for exchange

        cap_left  = pos_limit - position
        lot_ex = min(cap_left, max(15, int(edge * 8)))   # EDIT A         
        
  

        if not self.in_trend and sunlight >= CSI:
            self.in_trend = True
        elif self.in_trend and sunlight < CSI  - PARAMS[MAGNIFICENT_MACARONS]["SUN_BUFFER"]: 
            self.in_trend = False
...
################################################################################################################
        ...
        if self.in_trend and cap_left > 0:
            vol_needed = min(lot_ex, cap_left)       # respect 75‑limit
            for px, vol in get_best_asks_to_fill_WITH_LEVELS(
                    MAGNIFICENT_MACARONS, state, vol_needed):
                result[MAGNIFICENT_MACARONS].append(
                    Order(MAGNIFICENT_MACARONS, px, +vol)
                )
                position  += vol                      # update running pos
                cap_left  -= vol
                if cap_left <= 0:
                    break

        # Flatten longs when regime flips off
        if (not self.in_trend) and position > 0:
            vol_to_sell = min(lot_ex, position)       # same size logic
            for px, vol in get_best_bids_to_fill_WITH_LEVELS(
                    MAGNIFICENT_MACARONS, state, vol_to_sell):
                result[MAGNIFICENT_MACARONS].append(
                    Order(MAGNIFICENT_MACARONS, px, -vol)
                )
                position -= vol
        ...
        # import from island, then sell locally
        if island_buy_price < local_best_bid and cap_left > 0:
            qty = min(10, cap_left)                # ≤ 10 by game rule
            conversions -= qty                     # negative = sell to chef
            result[MAGNIFICENT_MACARONS].append(
                Order(MAGNIFICENT_MACARONS, local_best_bid, -qty)
            )

        # buy locally, then export to island
        if island_sell_price > local_best_ask and position > -pos_limit:
            qty = min(10, pos_limit + position)    # ≤ 10
            conversions += qty                     # positive = buy from chef
            result[MAGNIFICENT_MACARONS].append(
                Order(MAGNIFICENT_MACARONS, local_best_ask, +qty)
            )
