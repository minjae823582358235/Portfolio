    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for product in state.order_depths:
            ...
            if return_t1 > 0 and return_th > 0 and trend == "Uptrend":
                kelpoorder.append(Order(product, best_ask, 2))  # Strong bullish signal
            elif return_t1 < 0 and return_th > 0 and trend == "Uptrend":
                kelpoorder.append(Order(product, best_ask, -1))  # Adjustment signal
            elif return_t1 > 0 and return_th > 0 and trend == "Downtrend":
                kelpoorder.append(Order(product, best_ask, 0))  # Weak bullish signal

            if return_t1 < 0 and return_th < 0 and trend == "Downtrend":
                kelpoorder.append(Order(product, best_bid, -2))  # Strong bearish signal
            elif return_t1 > 0 and return_th < 0 and trend == "Downtrend":
                kelpoorder.append(Order(product, best_bid, 1))  # Strong bearish signal
            elif return_t1 < 0 and return_th < 0 and trend == "Uptrend":
                kelpoorder.append(Order(product, best_bid, 0))  # Weak bearish signal

            if trend == "Uptrend":
                kelpoorder.append(Order(product, best_ask, 0))  # Additional weak uptrend order
            elif trend == "Downtrend":
                kelpoorder.append(Order(product, best_bid, 0))  # Additional weak downtrend order

            result[product] = kelpoorder
