...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        def PriceAdjustment(product, mode=None):
            ...
            # --------------------------------------------------------------------
            holdFactor = 28.96446253617450  ## TODO OPTIMISE
            holdPremium = int(holdFactor * self.previouspositionCounter[product])

            if product == "SQUID_INK":
                S_high = 26.244155027497000 + holdPremium  # TODO OPTIMISE
                S_mid = 3.5707717204652600 + holdPremium  # TODO OPTIMISE
                S_low = 1.017147412182790 + holdPremium  # TODO OPTIMISE
                S_neg = -19.168886133394400 + holdPremium  # TODO OPTIMISE
                if mode == "buy":
                    factor = 1
                if mode == "sell":
                    factor = -1
            ...
            if product == "KELP":
                K_high = 35.5610483867722 + holdPremium  # TODO OPTIMISE
                K_mid = 34.49889192889150 + holdPremium  # TODO OPTIMISE
                K_low = 9.113029964595070 + holdPremium  # TODO OPTIMISE
                K_neg = 1.2853198109687800 + holdPremium  # TODO OPTIMISE
                if mode == "buy":
                    factor = 1
                if mode == "sell":
                    factor = -1
        ...
        # REDEFINE CLEAN LATER
        def avellaneda_stoikov(product, mid, inventory):
            """
            Calculate bid and ask prices around the mid-price/reservation price whilst adjusting based on current inventory.
            """
            # Reservation price - adjust the mid price based on inventory risk
            reservation_price = mid - inventory *2.6345501484152500 * (2.366**2) * 1

            # Optimal spread - adjusted based on the risk aversion parameter and order intensity
            optimal_spread = (2 / 2.6345501484152500) * np.log(1 + 2.6345501484152500 / 15.380396317284400)
